...
export function FeedbackForm() {
  ...
  async function handleSubmit() {
    ...
    setPending(true);
    try {
      const response = await fetch(clientApiPath("/feedback"), {
        method: "POST",
        headers: {
  ...
  return (
    <div className="space-y-6">
      <Textarea
        ...
        className="min-h-60 resize-none rounded-sm bg-background/70 px-7 py-6 text-sm leading-8 placeholder:text-sm placeholder:text-muted-foreground/80"
      />
      <div className="space-y-10">
        <span className="block font-mono text-base font-medium text-muted-foreground">
          {content.length}/{MAX_FEEDBACK_LENGTH}
        </span>
        <Button disabled={pending} onClick={handleSubmit} className="h-14 w-full rounded-md text-lg font-bold text-white hover:bg-[#f4a789]">
          <Send />
          提交反馈
        </Button>
      </div>
